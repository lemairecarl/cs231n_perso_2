...
def gradient_check():
  ...
  for param_name in sorted(grads):
    ...
    param_grad_num = eval_numerical_gradient(f, model.params[param_name], verbose=False, h=h, pname=param_name)
    ...
    if e > 1e-10:
      #mean_val = np.mean(np.abs(grads[param_name]))
  ...
  print '{:<10} {:<13} {:<13} {:<15}           {:<13} {:<13} {}'.format('Param', 'Error', 'Rescaled', '', 'Ana', 'Num', 'Kinks')
  for p in sorted(results):
    msg = gradient_check_message(results[p][1])
    print '{:<10} {:<13e} {:<13e} {:<15}   avgval: {:<13e} {:<13e} {}'.format(p, results[p][0], results[p][1], msg, avg[p][0], avg[p][1], kinks[p])
